...
class PerformanceMetrics:
    ...
    def calculate_f_number(
        self, entrance_pupil_diameter: Optional[float] = None
    ) -> Optional[float]:
        """
    ...
    def calculate_numerical_aperture(
        self, half_angle_deg: Optional[float] = None, medium_index: float = 1.0
    ) -> Optional[float]:
        """
    ...
    def estimate_resolution_rayleigh(
        self, wavelength: float = WAVELENGTH_GREEN, f_number: Optional[float] = None
    ) -> Optional[float]:
        """
    ...
    def calculate_depth_of_field(
        self, object_distance: float, circle_of_confusion: float = 0.03
        ...
        return {"near": near, "far": far, "total": dof, "hyperfocal": H}
    ...
    def get_all_metrics(
        ...
        if self.lens:
            ...
            if self.system.elements:
                metrics["entrance_pupil_diameter"] = (
                    entrance_pupil_diameter or self.system.elements[0].lens.diameter
                )
