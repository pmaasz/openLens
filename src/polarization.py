...
class PolarizationState:
    ...
    def elliptical(cls, a: float, b: float, angle_deg: float = 0):
        ...
        # Rotation matrix
        jones = np.array([a * cos_a - 1j * b * sin_a, a * sin_a + 1j * b * cos_a])
...
class JonesMatrices:
    ...
    def linear_polarizer(angle_deg: float) -> np.ndarray:
        ...
        cos_t = np.cos(theta)
        sin_t = np.sin(theta)
        return np.array([[cos_t**2, cos_t * sin_t], [cos_t * sin_t, sin_t**2]])
    ...
    def retarder(phase_deg: float, fast_axis_angle_deg: float = 0) -> np.ndarray:
        ...
        retarder = np.array([[np.exp(-1j * delta / 2), 0], [0, np.exp(1j * delta / 2)]])
...
class PolarizationCalculator:
    ...
    def birefringence_retardance(
        self, delta_n: float, thickness_mm: float, wavelength_nm: float
    ) -> float:
        """
