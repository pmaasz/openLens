...
class TestAssemblyGhostAnalysis(unittest.TestCase):
    ...
    def test_ghost_trace_multi_element(self):
        ...
        for g in ghosts:
            print(
                f"Path ({g.reflection_1_index}, {g.reflection_2_index}) - Intensity: {g.intensity:.2e}"
            )
            self.assertGreater(len(g.rays), 0)

        # Verify a specific path (e.g., reflection between lenses: 2 to 1)
        found_inter_element = any(
            g.reflection_1_index == 2 and g.reflection_2_index == 1 for g in ghosts
        )
        ...
        self.assertTrue(
            found_inter_element,
            "Did not find inter-element ghost path (L2 Front -> L1 Back)",
        )
