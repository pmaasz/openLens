...
class TestBoundaryConditions(unittest.TestCase):
    ...
    def test_total_internal_reflection(self):
        ...
        ray = Ray(x=0, y=0, angle_rad=math.pi / 4)
        # Going from glass to air at steep angle
        success = ray.refract_or_reflect(n1=1.5, n2=1.0, surface_normal_angle=0)
        # May fail or reflect depending on angle
        self.assertIn(success, (RefractionResult.REFRACTED, RefractionResult.REFLECTED))

    def test_same_refractive_indices(self):
        """Test refraction with same refractive index on both sides"""
        ray = Ray(x=0, y=0, angle_rad=math.pi / 4)
        initial_angle = ray.angle_rad
        success = ray.refract_or_reflect(n1=1.5, n2=1.5, surface_normal_angle=0)
        self.assertIs(success, RefractionResult.REFRACTED)
...
class TestInvalidInputHandling(unittest.TestCase):
    ...
    def test_nan_input(self):
        ...
        try:
            lens = Lens(radius_of_curvature_1=float("nan"), material="Custom")
            focal_length = lens.calculate_focal_length()
...
class TestNumericalStability(unittest.TestCase):
    ...
    def test_ray_at_lens_edge(self):
        ...
        # Ray at edge of lens
        ray = Ray(x=-50, y=24.9, angle_rad=0)  # Just inside edge
