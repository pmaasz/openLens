...
class TestRay(unittest.TestCase):
    ...
    def test_ray_total_internal_reflection(self):
        ...
        self.assertIs(
            success,
            RefractionResult.REFLECTED,
            "Should reflect just above critical angle",
        )
...
class TestLensRayTracer(unittest.TestCase):
    ...
    def test_parallel_rays_converge(self):
        ...
        for ray in rays:
            self.assertGreater(
                len(ray.path), 1
            )  # Ray should have at least initial and final points
    ...
    def test_point_source_rays(self):
        ...
        rays = tracer.trace_point_source_rays(
            source_x=-100, source_y=0, num_rays=7, max_angle_deg=15.0
        )
