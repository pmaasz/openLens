...
class TestGhostAnalysis(unittest.TestCase):
    ...
    def test_ghost_path_detection(self):
        ...
        self.assertTrue(
            found_internal_ghost,
            "Did not find expected internal ghost (Back->Front reflection)",
        )

    def test_ghost_path_validity(self):
        """Trace a specific ghost and verify ray path length."""
        # Trace specifically 1->0
        ghost = self.analyzer._trace_ghost_path(
            1, 0, num_rays=1, wavelength=WAVELENGTH_GREEN * NM_TO_MM
        )
