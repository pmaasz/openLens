...
if OPTIONAL_DEPS_AVAILABLE:
    from src.interactive_ray_tracer import (
...
class MockOpticalSystem:
    ...
    def get_aberrations(self, wavelength):
        return {"spherical": 0.1, "coma": 0.05, "astigmatism": 0.03}
...
class TestImageSimulator(unittest.TestCase):
    ...
    def test_simulate_image(self):
        ...
        result = self.simulator.simulate_image(input_image, object_distance=100.0)
    ...
    def test_test_patterns(self):
        ...
        patterns = ["grid", "checkerboard", "star", "siemens_star", "slant_edge"]

        for pattern_name in patterns:
            result = self.simulator.simulate_test_pattern(
                pattern_name, size=(128, 128), object_distance=100.0
            )
            assert "output_image" in result
...
class TestMechanicalDesigner(unittest.TestCase):
    ...
    def test_calculate_spacers(self):
        ...
        spacers = self.designer.calculate_spacers(target_spacing=[10.0], tolerance=0.1)
