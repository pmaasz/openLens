...
class TestAberrationsCalculator(unittest.TestCase):
    ...
    def test_zero_power_lens_handling(self):
        ...
        self.assertTrue(
            "error" in results
            or results["focal_length"] is None
            or abs(results["focal_length"]) > 1e6
        )
...
class TestAberrationsBehavior(unittest.TestCase):
    ...
    def test_aberrations_scale_correctly_with_parameters(self):
        ...
        ratio = abs(results_large["spherical_aberration"]) / abs(
            results_small["spherical_aberration"]
        )
    ...
    def test_extreme_field_angles(self):
        ...
        lens = Lens(
            ...
            thickness=5.0,  # Asymmetric meniscus
            diameter=50.0,
            refractive_index=1.5168,
            material="Custom",
        )
...
class TestAberrationPhysicsIdentities(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.results = AberrationsCalculator(self.lens).calculate_all_aberrations(
            field_angle_deg=5.0
        )
    ...
    def test_f_number_is_f_over_diameter(self):
        ...
        self.assertAlmostEqual(self.results["f_number"] / expected, 1.0, places=12)
