...
class TestEnvironmental(unittest.TestCase):
    ...
    def test_air_index(self):
        ...
        # 587.6 nm (d-line)
        n_air = EnvironmentalAnalyzer.calculate_air_index(
            587.6, temperature_c=20.0, pressure_atm=1.0
        )
        ...
        n_hot = EnvironmentalAnalyzer.calculate_air_index(
            587.6, temperature_c=100.0, pressure_atm=1.0
        )
        self.assertLess(n_hot, n_air)

        # Test Pressure dependence (Higher P -> More dense -> Higher index)
        n_high_p = EnvironmentalAnalyzer.calculate_air_index(
            587.6, temperature_c=20.0, pressure_atm=2.0
        )
    ...
    def test_thermal_expansion(self):
        ...
        new_sys = EnvironmentalAnalyzer.apply_environment(
    ...
    def test_pressure_effect_on_index(self):
        ...
        new_sys = EnvironmentalAnalyzer.apply_environment(
            self.sys, temperature_c=20.0, pressure_atm=2.0
        )
        new_lens = new_sys.elements[0].lens

        # Calculate expected change
        n_air_1 = EnvironmentalAnalyzer.calculate_air_index(
            587.6, temperature_c=20.0, pressure_atm=1.0
        )
        n_air_2 = EnvironmentalAnalyzer.calculate_air_index(
            587.6, temperature_c=20.0, pressure_atm=2.0
        )
