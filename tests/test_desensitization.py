...
def _make_tolerances():
    return [
        ToleranceOperand(element_index=0, param_type=ToleranceType.RADIUS_1,
                         min_val=-0.5, max_val=0.5),
        ToleranceOperand(element_index=0, param_type=ToleranceType.THICKNESS,
...
class TestRobustMeritFunction(unittest.TestCase):
    ...
    def setUp(self):
        self.targets = [OptimizationTarget("focal_length", 100.0,
                                           target_type="target")]
    ...
    def test_sensitivity_weight_inflates_merit(self):
        ...
        nominal = RobustMeritFunction(_make_system(), self.targets,
                                      self.tolerances,
                                      sensitivity_weight=0.0)
        weighted = RobustMeritFunction(_make_system(), self.targets,
                                       self.tolerances,
                                       sensitivity_weight=5.0)
        self.assertGreaterEqual(weighted.evaluate(_make_system()),
                                nominal.evaluate(_make_system()))
...
class TestDesensitizationOptimizer(unittest.TestCase):
    ...
    def test_optimize_robust_returns_result_and_keeps_geometry_valid(self):
        variables = [OptimizationVariable("R1", 0, "radius_of_curvature_1",
                                          100.0, 80.0, 120.0)]
        targets = [OptimizationTarget("focal_length", 100.0,
                                      target_type="target")]
